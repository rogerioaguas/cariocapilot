...
class Tici(HardwareBase):
    ...
    def get_sim_info(self):
        ...
        if sim_path == "/":
            ...
            return {
                ...
                'network_type': ["Unknown"],
                'sim_state': ["READY"],
                'data_connected': modem.Get(MM_MODEM, 'State', dbus_interface=DBUS_PROPS,
                                            timeout=TIMEOUT) == MM_MODEM_STATE.CONNECTED,
            }
    ...
    def set_bandwidth_limit(upload_speed_kbps: int, download_speed_kbps: int) -> None:
        ...
        upload = [
            ...
            (True, tc + ["class", "add", "dev", adapter, "parent", "1:", "classid", "1:20", "htb", "rate",
                         f"{upload_speed_kbps}kbit"]),

            # Create universal 32 bit filter on adapter that sends all outbound ip traffic through the class
            (True,
             tc + ["filter", "add", "dev", adapter, "parent", "1:", "protocol", "ip", "prio", "10", "u32", "match",
        ...
        download = [
            ...
            (True,
             tc + ["filter", "add", "dev", adapter, "parent", "ffff:", "protocol", "ip", "u32", "match", "u32", "0",
                   "0", "action", "mirred", "egress", "redirect", "dev", ifb]),

            # Add class and rules for virtual interface
            (True, tc + ["qdisc", "add", "dev", ifb, "root", "handle", "2:", "htb"]),
            (True, tc + ["class", "add", "dev", ifb, "parent", "2:", "classid", "2:1", "htb", "rate",
                         f"{download_speed_kbps}kbit"]),

            # Add filter to rule for IP address
            (True,
             tc + ["filter", "add", "dev", ifb, "protocol", "ip", "parent", "2:", "prio", "1", "u32", "match", "ip",
                   "src", "0.0.0.0/0", "flowid", "2:1"]),
        ]
    ...
    def get_modem_nv(self):
        ...
        try:
            modem = self.get_modem()
            return {
                fn: str(modem.Command(f'AT+QNVFR="{fn}"', math.ceil(timeout), dbus_interface=MM_MODEM, timeout=timeout))
                for
                fn in files}
        except Exception:
            return None
    ...
    def initialize_hardware(self):
        ...
        # is_enabled_hotspot = Params().get_bool("IsEnabledHotspot")
        # if is_enabled_hotspot:
